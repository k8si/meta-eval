...
object RPPParser {
  ...
  def year(bib: Element): Year = bib.findText("date>year").extractYear()

  //    bib.findAttributeValue("monogr>imprint>date[type=published]", "when").extractYear

  def author(e: Element): Option[Author] = Author(
    lastName = e.findText("person-last").toName,
    firstName = e.findText("person-first").toName,
    middleNames = e.select("person-middle").map(_.text.toName)
  ).ifDefined
  ...
  private def extractMetadata(doc: Element): PaperMetadata = {
    val title = Title(doc.findText("header>title"))
    val authors = doc.select("header>authors>person").flatMap(author)
    val year = doc.findText("header>date").extractYear
    val venue = Venue(doc.findText("header>affiliation"))
    PaperMetadata(title = title, authors = authors, year = year, venue = venue)
  }

  private def extractBibEntriesWithId(doc: Element) =
    for {
      bib <- doc.select("references>reference")
    } yield {
      //val id = bib.attr("xml:id")
      val id = bib.findText("marker")
      //      val title = bib.findText("analytic>title[type=main]").toTitle match {
      //        case "" => Title(bib.findText("monogr>title"))
      //        case s => Title(s)
      //      }
      val title = Title(bib.findText("title"))
      ...
      //        case l => l
      //      }
      val authors = bib.select("authors>person").flatMap(author)
      //      val venue = Venue(bib.findText("monogr>title"))
      val venue = {
        var found = ""
        found = bib.findText("venue>journal")
        if (found.equals("")) {
          found = bib.findText("venue>booktitle")
        }
        Venue(found)
      }
      //      val yr = year(bib)
  ...
  def extractStructuredDocument(xmlString: String): StructuredDocument = {
    ...
    val paperAbstract = doc.select("header>abstract").headOption.map(_.text)
